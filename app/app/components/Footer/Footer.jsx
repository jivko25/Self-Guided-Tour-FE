...
export default function Footer() {
  ...
  const hiddenFooter = isSpecialPage
    ...
    : "web:flex tablet:flex";

  const footerColors = isTourPage
    ? "text-[#13294b] bg-white"
    : "text-white bg-[#13294b]";

  const iconColors = isTourPage ? "text-[#13294b]" : "text-white";

  return (
    <div
      className={`hidden web:py-[0%] tablet:flex tablet:py-[10%] flex-col items-center justify-center w-full min-h-[474px] web:px-[20%] tablet:px-[3%] ${footerColors} ${hiddenFooter}`}
    >
      ...
      <section className="flex w-full max-w-[1336px] web:gap-[100px] tablet:gap-[30px]">
        ...
        <div className="flex flex-col flex-1">
          ...
          <div className="flex flex-col gap-[10px]">
            <Link
              href="/explore?page=1&search=sofia"
              className="text-base font-normal   cursor-pointer hover:underline"
            >
              Sofia
            </Link>
            <Link
              href="/explore?page=1&search=plovdiv"
              className="text-base font-normal   cursor-pointer hover:underline"
            >
              Plovdiv
            </Link>
            <Link
              href="/explore?page=1&search=veliko%20tarnovo"
              className="text-base font-normal   cursor-pointer hover:underline"
            >
              Veliko Tarnovo
            </Link>
            <Link
              href="/explore?page=1&search=ruse"
              className="text-base font-normal   cursor-pointer hover:underline"
            >
              Ruse
            </Link>
            <Link
              href="/explore?page=1&search=burgas"
              className="text-base font-normal   cursor-pointer hover:underline"
            >
              Burgas
            </Link>
        ...
        <div className="flex flex-col flex-1">
          <p className="text-base font-medium   mb-[20px]">Stay in Touch</p>
